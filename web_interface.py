...
class WebTradingAnalyzer:
    def __init__(self):
        ...
        self.data_dir = Path(__file__).parent / "benchmark"
    ...
    def get_available_files(self, asset: str, timeframe: str) -> list:
        ...
        # Try the timeframe-specific path first, e.g., benchmark/1h/btc
        timeframe_path = self.data_dir / timeframe / asset.lower()
        # Fallback to the general asset path, e.g., benchmark/btc
        general_path = self.data_dir / asset.lower()

        asset_dir = None
        if timeframe_path.exists():
            asset_dir = timeframe_path
        else:
            # If the specific path doesn't exist, use the general one
            asset_dir = general_path

        if not asset_dir.exists():
            return []

        pattern = f"{asset.upper()}_{timeframe}_*.csv"
...
def analyze():
    try:
        ...
        if data_source == "local":
            ...
            if not file_name:
                return jsonify({"error": "File name is required for local data source."})
            
            # The get_available_files function returns pathlib.Path objects, so we can use them directly
            # We must reconstruct the path relative to the app's root
            ...
            return jsonify({"error": "Invalid data source specified."})
...
def get_files(asset, timeframe):
    ...
    try:
        ...
        for i, file_path in enumerate(files):
            ...
            file_list.append(
                {"index": i, "number": file_number, "name": str(file_path), "displayName": file_path.name}
            )
